...
##--Vars---##
INPUT_RESOURCE_DIRECTORY = "resources/"
OUTPUT_RESOURCE_DIRECTORY = "data/"
    ...
    with open('data/tempout.txt', 'w') as out:
        for f in file_list:
            out.write("image name: " + f + "\n\n" +
                      pytesseract.image_to_string(Image.open(resource_directory + "/" + f), lang="eng").encode(
                          'utf-8').strip() + "\n\n")
...
def main():
    ...
    print
    "Function processing time %.4f seconds" % (function_end,)
